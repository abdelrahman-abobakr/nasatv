...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100 bg-gray-50/50">
            <h3 class="text-lg font-bold text-gray-800">Edit Subscription Tier</h3>
            <p class="text-xs text-gray-500">Modify the service package details and features</p>
        </div>
        <div class="p-8">
            <form method="POST" action="{{ route('admin.plans.update', $plan) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="duration" id="duration" value="{{ old('duration', $plan->duration) }}" placeholder="e.g. 1 Month, 12 Months, Lifetime" required
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Briefly describe the benefits of this tier...">{{ old('description', $plan->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="relative">
                            <select name="status" id="status" required
                                ...
                                <option value="active" {{ old('status', $plan->status) == 'active' ? 'selected' : '' }}>Publicly Active</option>
                                <option value="inactive" {{ old('status', $plan->status) == 'inactive' ? 'selected' : '' }}>Hidden / Internal</option>
                            </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="simultaneous_devices" id="simultaneous_devices" value="{{ old('simultaneous_devices', $plan->simultaneous_devices ?? 1) }}" min="1"
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" name="channels" id="channels" value="{{ old('channels', $plan->channels) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="movies" id="movies" value="{{ old('movies', $plan->movies) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="series" id="series" value="{{ old('series', $plan->series) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="max_resolution" id="max_resolution"
                            ...
                            <option value="SD" {{ old('max_resolution', $plan->max_resolution) == 'SD' ? 'selected' : '' }}>SD (480p)</option>
                            <option value="HD" {{ old('max_resolution', $plan->max_resolution) == 'HD' ? 'selected' : '' }}>HD (720p)</option>
                            <option value="FHD" {{ old('max_resolution', $plan->max_resolution) == 'FHD' ? 'selected' : '' }}>Full HD (1080p)</option>
                            <option value="4K" {{ old('max_resolution', $plan->max_resolution) == '4K' ? 'selected' : '' }}>Ultra HD (4K)</option>
                            <option value="8K" {{ old('max_resolution', $plan->max_resolution) == '8K' ? 'selected' : '' }}>8K Resolution</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="bg-gray-50/50 p-6 rounded-xl border border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                    <div class="flex items-center justify-between group">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="match_recording" value="1" {{ old('match_recording', $plan->match_recording) ? 'checked' : '' }} class="sr-only peer">
                    ...
                    <div class="flex items-center justify-between group">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="multi_audio_subtitles" value="1" {{ old('multi_audio_subtitles', $plan->multi_audio_subtitles) ? 'checked' : '' }} class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                        </label>
                    </div>
                    ...
                    <div class="flex items-center justify-between group">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="support_24_7" value="1" {{ old('support_24_7', $plan->support_24_7) ? 'checked' : '' }} class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                        </label>
                    </div>
                    ...
                    <div class="flex items-center justify-between group">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="instant_activation" value="1" {{ old('instant_activation', $plan->instant_activation) ? 'checked' : '' }} class="sr-only peer">
                            <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                        </label>
                    </div>
                </div>
                ...
                <div x-data="{ 
                    extras: {{ json_encode(old('extras', $plan->extras ?? [])) }},
